...
# Fix import path to ensure all modules can be found
...
# Import with try-except to handle potential errors gracefully
try:
    from utils.custom_logger import get_logger
    logging = get_logger()
except ImportError as e:
    print(f"Warning: Could not import custom logger: {e}")
    logging = logging.getLogger("streamlit")

try:
    from utils.neo4j_utils import get_graph_stats
except ImportError as e:
    print(f"Warning: Could not import neo4j_utils: {e}")
            ...
            if initial_retrieval_num > 1:
                ...
                with st.spinner("Finding the best matching sections..."):
                    try:
                        ...
                        if not enable_llm:
                            original_client = st.session_state.query_engine.gemini_client
                            st.session_state.query_engine.gemini_client = None
                        ...
                        if enable_llm and st.session_state.query_engine.gemini_client and results.get('status') == 'success':
                            try:
                                main_node = {
                                    'id': results['id'],
                                    'title': results['title'],
                                ...
                                llm_response = st.session_state.query_engine._call_gemini_api(prompt, model=llm_model, max_tokens=max_tokens)
                        ...
                        if not enable_llm:
                            st.session_state.query_engine.gemini_client = original_client
    ...
    if st.session_state.selected_candidate_id and st.session_state.last_results is None:
        with st.spinner("Generating answer from selected section…"):
            try:
                ...
                if enable_llm and st.session_state.query_engine.gemini_client and final_results.get('status') == 'success':
                    try:
                        mn = {
                            'id': final_results['id'],
                        ...
                        resp = st.session_state.query_engine._call_gemini_api(prmpt, model=llm_model if enable_llm else "gpt-4o-mini", max_tokens=max_tokens if enable_llm else 1500)
